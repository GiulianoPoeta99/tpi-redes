...
const TransferView: React.FC = () => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-900 p-6" onDragEnter={handleDrag} onDragOver={handleDrag} onDragLeave={handleDrag} onDrop={handleDrop}>
            ...
            <div className="w-full max-w-md space-y-6">
                    ...
                    <div className="flex bg-gray-900 rounded-lg p-1 w-full">
                        <button
                            ...
                            className={`flex-1 py-2 rounded-md text-sm font-medium transition-all ${
                                mode === 'server' ? 'bg-gray-700 text-white shadow' : 'text-gray-400 hover:text-white'
                            }`}
                        >
                            Receive (Server)
                        </button>
                        <button
                            ...
                            className={`flex-1 py-2 rounded-md text-sm font-medium transition-all ${
                                mode === 'client' ? 'bg-gray-700 text-white shadow' : 'text-gray-400 hover:text-white'
                            }`}
                        >
                            Send (Client)
                        </button>
                ...
                <div className="bg-gray-800 p-8 rounded-2xl border border-gray-700 shadow-xl relative overflow-hidden transition-all duration-300">
                    ...
                    <div className="space-y-5">
                        {/* Protocol */}
                        <div>
                            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Protocol</label>
                            <div className="grid grid-cols-2 gap-3">
                                <label className={`cursor-pointer border border-gray-700 rounded-lg p-3 flex items-center justify-center gap-2 transition-all ${protocol === 'tcp' ? 'bg-blue-600/20 border-blue-500' : 'bg-gray-900 hover:border-gray-600'}`}>
                                    <input type="radio" className="hidden" checked={protocol === 'tcp'} onChange={() => setProtocol('tcp')} />
                                    <span className={`text-sm font-medium ${protocol === 'tcp' ? 'text-blue-400' : 'text-gray-400'}`}>TCP</span>
                                </label>
                                <label className={`cursor-pointer border border-gray-700 rounded-lg p-3 flex items-center justify-center gap-2 transition-all ${protocol === 'udp' ? 'bg-purple-600/20 border-purple-500' : 'bg-gray-900 hover:border-gray-600'}`}>
                                    <input type="radio" className="hidden" checked={protocol === 'udp'} onChange={() => setProtocol('udp')} />
                                    <span className={`text-sm font-medium ${protocol === 'udp' ? 'text-purple-400' : 'text-gray-400'}`}>UDP</span>
                                </label>
                            </div>
                        </div>
                        ...
                        {/* Port (Compact) */}
                        <div>
                             <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Port</label>
                             <input 
                                type="number" 
                                value={port}
                        ...
                        {/* Client Inputs */}
                         {mode === 'client' && (
                            <div className="space-y-4 animate-slide-down">
                                <div>
                                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Destination IP</label>
                                        ...
                                        {showPeers && discoveredPeers.length > 0 && (
                                            <div className="absolute z-10 w-full mt-1 bg-gray-800 border border-gray-600 rounded-lg shadow-xl max-h-40 overflow-y-auto">
                                                {discoveredPeers.map((peer, idx) => (
                                                    <button key={idx} onClick={() => { setIp(peer.ip); setPort(peer.port); setShowPeers(false); }} className="w-full text-left px-4 py-2 hover:bg-gray-700 text-sm text-gray-200 border-b border-gray-700 last:border-0">
                                                        <span className="font-bold">{peer.hostname}</span> <span className="opacity-70 text-xs">({peer.ip})</span>
                                                    </button>
                                                ))}
                                            </div>
                                        )}
