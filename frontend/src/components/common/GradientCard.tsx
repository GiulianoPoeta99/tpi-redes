...
const GradientCard: React.FC<GradientCardProps> = ({ 
  ...
  return (
    <div className={`bg-gradient-to-r ${gradients[variant]} border p-6 rounded-2xl flex items-center justify-between shadow-lg`}>
      <div>
        <h2 className="text-2xl font-bold text-white flex items-center gap-3">
          <Icon className={iconColorClass} size={28} />
          {title}
        </h2>
        <p className={`${variant === 'red' ? 'text-red-200/60' : 'text-blue-200/60'} mt-1 text-sm max-w-lg`}>
          {description}
        </p>
      </div>
      {children && (
