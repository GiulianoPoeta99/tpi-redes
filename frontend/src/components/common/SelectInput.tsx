  ...
  return (
    <div className={`relative group ${className}`} ref={containerRef}>
      ...
      >
        ...
        {/* Start Icon */}
        {icon && (
            <div className={`absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none transition-colors ${disabled ? 'text-gray-500' : 'text-blue-400'}`}>
            {icon}
            </div>
        )}
      ...
      {/* DROPDOWN MENU */}
      {isOpen && !disabled && (
        <div className={`absolute top-[calc(100%+4px)] ${align === 'right' ? 'right-0 origin-top-right' : 'left-0 origin-top-left'} min-w-full w-max z-50 animate-in fade-in zoom-in-95 duration-100`}>
