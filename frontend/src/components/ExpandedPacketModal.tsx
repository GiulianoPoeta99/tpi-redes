      ...
      <div className="h-full flex flex-col bg-gray-900/50 rounded-lg overflow-hidden border border-gray-700">
        <TableVirtuoso
          ...
          itemContent={(index, pkt) => {
             ...
             return (
            <>
              <td className="p-2 px-3 text-gray-500 font-mono text-xs border-b border-gray-800/50">{index + 1}</td>
              <td className="p-2 px-3 text-gray-400 font-mono text-xs border-b border-gray-800/50">{formatTime(pkt.timestamp)}</td>
              <td className="p-2 px-3 font-mono text-xs border-b border-gray-800/50 text-blue-300">{src.ip}</td>
              <td className="p-2 px-3 text-gray-400 font-mono text-xs border-b border-gray-800/50">{src.port}</td>
              <td className="p-2 px-3 font-mono text-xs border-b border-gray-800/50 text-purple-300">{dst.ip}</td>
              <td className="p-2 px-3 text-gray-400 font-mono text-xs border-b border-gray-800/50">{dst.port}</td>
              <td className={`p-2 px-3 font-mono text-xs border-b border-gray-800/50 ${getProtocolClass(pkt.protocol)}`}>{pkt.protocol}</td>
              <td className="p-2 px-3 font-mono text-xs border-b border-gray-800/50">
                 {pkt.flags ? (
                    <span className="bg-gray-700 text-gray-300 px-1.5 py-0.5 rounded text-[10px]">{pkt.flags}</span>
                 ) : '-'}
              </td>
              <td className="p-2 px-3 text-gray-400 font-mono text-xs border-b border-gray-800/50">{pkt.seq}</td>
              <td className="p-2 px-3 text-gray-400 font-mono text-xs border-b border-gray-800/50">{pkt.ack}</td>
              <td className="p-2 px-3 text-gray-400 font-mono text-xs border-b border-gray-800/50">{pkt.window}</td>
              <td className="p-2 px-3 text-gray-400 font-mono text-xs border-b border-gray-800/50">{pkt.length}</td>
              <td className="p-2 px-3 text-gray-500 font-mono text-xs border-b border-gray-800/50 truncate max-w-xs" title={pkt.info}>
                {pkt.info}
              </td>
            </>
          )}}
